...
static int	initialize_philos(t_table *table, int *philos_initialized)
{
	int	i;
...
int	initialize_simulation(t_table *table)
{
	...
	if (!table->forks || init_forks(table, &table->forks_initialized) != 0)
		return (cleanup(table, table->forks_initialized, 0, 1));
	if (init_shared_mutexes(table, table->forks_initialized) != 0)
		return (cleanup(table, table->forks_initialized, 0, 1));
	table->start_time = get_current_time() + 100;
	table->philos = malloc(sizeof(t_philo) * table->philo_count);
	if (!table->philos || initialize_philos(table, &table->philos_initialized) != 0)
		return (cleanup_shared_mutexes(table),
			cleanup(table, table->forks_initialized, table->philos_initialized, 1));
	return (0);
}
